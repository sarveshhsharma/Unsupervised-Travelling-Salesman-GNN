...
def get_heat_map(SctOutput,num_of_nodes,device = 'cuda'):
    ...
    HeatM = torch.matmul(SctOutput, torch.roll(torch.transpose(SctOutput, 1, 2),-1, 1))
    return HeatM
#This transposes the last two dimensions of SctOutput
#torch.roll(x,-1,1) = This shifts all rows up(-1) by 1 (circularly)
# HeatM[i] = SctOutput[i] X rolled(SctOutput[i].T)
